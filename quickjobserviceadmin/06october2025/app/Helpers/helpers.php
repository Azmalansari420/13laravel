  ...
  function purchasestatus($value)
    {
      if($value==1)
      {
        $string = '<p class="mb-0 text-dark font-weight-bold d-flex justify-content-start align-items-center">
                      <small>
                        <svg class="mr-2" xmlns="http://www.w3.org/2000/svg" width="18" viewBox="0 0 24 24" fill="none">        
                          <circle cx="12" cy="12" r="8" fill="#000"></circle>
                        </svg>
                      </small>Pending
                   </p>';
      }
...
function custom_meta_tags($slug = '')
  ...
  function cityname($id)
    {
        return DB::table('city')->where('id', $id)->value('name') ?? '';
    }

  function categoryname($id)
    {
        return DB::table('category')->where('id', $id)->value('name') ?? '';
    }
  ...
  function jobtype($id)
    {
        return DB::table('job_type')->where('id', $id)->value('name') ?? '';
    }

  function jobskill($id)
    {
        return DB::table('skills')->where('id', $id)->value('name') ?? '';
    }


function hoursDifference($postDate) 
{
    $post = new DateTime($postDate);
    $now = new DateTime(); // current date and time

    $diff = $now->getTimestamp() - $post->getTimestamp(); // difference in seconds

    return floor($diff / 3600); // convert seconds to hours
}
